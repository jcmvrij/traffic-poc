...
COPY zuid-holland-latest.osm.pbf valhalla_tiles/zuid-holland.osm.pbf
...
RUN cd valhalla_tiles; valhalla_build_tiles -c valhalla.json zuid-holland.osm.pbf
...
# Let's update the traffic for https://www.openstreetmap.org/way/127838917
# Generate a csv with speeds for all edges
COPY update_traffic.py valhalla_tiles/traffic/update_traffic.py
RUN cd /valhalla_tiles/traffic; python3 update_traffic.py 127838917 /valhalla_tiles/valhalla_tiles/way_edges.txt
...
RUN cd /valhalla_tiles/traffic; \
    edge_id=`grep 127838917 /valhalla_tiles/valhalla_tiles/way_edges.txt | cut -d ',' -f3`; \
...
# Generate the traffic archive on Oude Waalsdorperweg 
RUN valhalla_traffic_demo_utils --config /valhalla_tiles/valhalla.json --generate-live-traffic 1/51304/0,20,`date +%s`
